  ...
  void addSon(st_node_ptr node, st_node_ptr son) {
    ...
    bool sameSons(st_node_ptr n1, st_node_ptr n2) {
      ...
      while (currentSon1->node == currentSon2->node) {
        if (currentSon1->nextSon == nullptr ||
            currentSon2->nextSon == nullptr) {
    ...
    st_node_ptr copyIfNotSimulated(st_node_ptr node, sharingtree & S,
      ...
      if (node->nextNode != nullptr) {
        ...
        while (son != nullptr) {
          S.addSon(newNode,
                   copyIfNotSimulated(son->node, S, nextLayer, newNode));
          son = son->nextSon;
        }
      }
      return addNode(destinationLayer, newNode);
    ...
    st_node_ptr node_intersect(st_node_ptr n_s, st_node_ptr n_t,
                               sharingtree & I, st_layer_ptr newLayer,
                               st_node_ptr father) {
      st_node_ptr newNode;
    ...
    std::map<std::vector<size_t>, std::vector<st_node_ptr>> buildLayer(
        st_layer_ptr layerK,
        std::map<std::vector<size_t>, std::vector<size_t>> & layerData, int k) {
      // Init result
