...
namespace posets::utils {
	...
	private:
      ...
      assert (node != nullptr);
		...
		*/
		...
		std::map<std::vector<size_t>, std::vector<st_node_ptr>> buildLayer(st_layer_ptr layerK, std::map<std::vector<size_t>, std::vector<size_t>>& layerData, int k) {
			// Init result
			std::map<std::vector<size_t>, std::vector<st_node_ptr>> resultNodes{};

			// We have built all layers, so we create the final layer with the EoL node
			if (k == 0) {
				...
				// Create a node for each partition and add the children from that partition
				for (auto const& [n, children] : childNodes) {
					...
					for (auto const& child : children) {
						...
						if (sameValueSon == nullptr) {
							...
							// If there already is a son with that value, remove it and compute a union-node of the two instead
							removeSon(newNode, sameValueSon);
							st_node_ptr unionSon = node_union(sameValueSon, child, *this, nextLayer);
	...
	public:
		...
		sharingtree(R&& elements) :
			maxLayers{static_cast<int>((*elements.begin()).size())},
			maxNodes{static_cast<int>((*elements.begin()).size() * (elements.size()) + 2)},
			maxSons{static_cast<int>((*elements.begin()).size() * (*elements.begin()).size() * elements.size())}
		{
			layerBuffer = new st_layer[maxLayers];
			nodeBuffer = new st_node[maxNodes];
			sonBuffer = new st_son[maxSons];
			...
			std::map<std::vector<size_t>, std::vector<size_t>> vectorData{ {pref, vectorIds} };
