...
template <Vector> class kdtree;

template <Vector V>
std::ostream &operator<<(std::ostream &os, const utils::kdtree<V> &f);

template <Vector V> class kdtree {
private:
  struct kdtree_node;
  ...
  void recursive_build(size_t result, // where to leave the new tree
    ...
    assert(static_cast<size_t>(4 << (int)(std::floor(std::log2(
                                   this->vector_set.size())))) > result);
    ...
    std::nth_element(
        begin_it, median_it, end_it, [this, &axis](size_t i1, size_t i2) {
    ...
    auto max_it = std::max_element(
        begin_it, median_it, [this, &axis](size_t i1, size_t i2) {
          return this->vector_set[i1][axis] < this->vector_set[i2][axis];
        });
    ...
    assert(static_cast<size_t>(std::distance(begin_it, median_it)) ==
           length / 2);
    assert(static_cast<size_t>(std::distance(median_it, end_it)) ==
           length - (length / 2));
    assert(static_cast<size_t>(std::distance(begin_it, median_it)) +
    ...
    recursive_build((result * 2) + 1, begin_it, median_it, length / 2,
                    next_axis);
    recursive_build((result * 2) + 2, median_it, end_it, length - (length / 2),
                    next_axis);
    return;
  }
  ...
  bool recursive_dominates(const V &v, bool strict, size_t node_idx,
    ...
    bool r_succ = recursive_dominates(v, strict, (2 * node_idx) + 2, lbounds,
                                      still_to_dom);
    if (r_succ)
      return true;
    ...
    return recursive_dominates(v, strict, (2 * node_idx) + 1, lbounds,
                               dims_to_dom);
  }
...
public:
  ...
  void relabel_tree(R &&elements, Proj proj = {}) {
    ...
    size_t oldsize =
        4 << (size_t)(std::floor(std::log2(this->vector_set.size())));
  ...
  kdtree(R &&elements, Proj proj = {})
      : dim(proj(*elements.begin()).size()), tree(nullptr) {
    relabel_tree(std::move(elements), proj);
  }

  kdtree(const kdtree &other) = delete;
  kdtree(kdtree &&other)
      : dim(other.dim), tree(other.tree),
